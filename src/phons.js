export function phons (phon) {
  ...
  for (let i = 0; i < f.length; i++) {
    const Lp = getLp(phon, Tf[i], Lu[i], af[i])
    result.push({ Lp, f: f[i] })
  }

  return result
}

function getLp (Ln, Tf, Lu, af) {
  const Af = 4.47E-3 * (Math.pow(10., 0.025 * Ln) - 1.15) + Math.pow(0.4 * Math.pow(10., ((Tf + Lu) / 10) - 9), af)
